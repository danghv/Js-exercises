...
// ham upper letter
var upperFirstLetter = function upperFirstLetter(str) {
	var strBuff = str.split(" ");
	var buff;
...
var searchMaxWord = function searchMaxWord(str) {
	var strBuff = str.split(" ");
	var maxWord = "";
	var buff;
	for (var i = 0; i < strBuff.length; i++) {
		if (checkString(strBuff[i]) === false) {
			buff = strBuff[i].replace(/\.|\,|\:|\;/g, " ").split(" ");
			// console.log(buff);
			for (var j = 0; j < buff.length; j++) {
				if (buff[j].length > maxWord.length) {
					maxWord = buff[j];
				}
				// console.log(maxWord);
...
// console.log(searchMaxWord("hi 1 12 123 1234 thereee, minh.la.abcdefghijklmnmmmmm test"));
exports.reverseString = reverseString;
exports.upperFirstLetter = upperFirstLetter;
exports.searchMaxWord = searchMaxWord;
